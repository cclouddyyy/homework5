...
public class HomeWork5 {
    ...
    public static class Person {
        ...
        protected Person(String name, Integer health, Integer protection) {
            this.name = name;
            this.protection = 0;
        }
        ...
        public String announce() {
            return "Person " + this.name + " имеет характеристики: " + getHealth() + " здоровья и " + this.protection + " защиты";
        }
        public void takeDamage(int damage) throws Exception {
            if (damage < 0) {
                throw new Exception("Урон не может быть отрицательным");
            }
            int newHealth = health - (damage - protection);
            if (damage >= newHealth) {
                health = 0;
                throw new PersonDead("Здоровья погибшим");
            } else {
                health = newHealth;
            }
        }
